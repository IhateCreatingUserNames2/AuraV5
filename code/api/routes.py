...
# Dependencies
from api.deps import get_repository, get_active_ceaf_system, get_ceaf_factory, get_db, verify_token, get_agent_manager, JWT_SECRET, JWT_ALGORITHM
...
def create_access_token(user_id: str, username: str) -> str:
    # Usa as constantes importadas de deps ou definidas aqui
...
async def chat_endpoint(
    ...
    messages = await repo.get_session_messages(session_id, limit=30)
...
async def create_agent_endpoint(
        ...
        current_user: dict = Depends(verify_token), # Agora funciona vindo de deps
        repo: AgentRepository = Depends(get_repository),
        agent_manager: AgentManager = Depends(get_agent_manager) # Injeção funcionando
):
    # 1. Geramos o ID aqui para garantir que SQL e Arquivos tenham o mesmo ID
    agent_id = str(uuid.uuid4())
    user_id = current_user["user_id"]

    # 2. Cria no Banco de Dados Relacional (SQL)
    new_agent_sql = await repo.create_agent(
        agent_id=agent_id,
        user_id=user_id,
        name=request.name,
    ...
    # 3. Prepara a Memória de Identidade (A "Alma")
    initial_self_model = CeafSelfRepresentation(
        persona_attributes={
            "tone": "helpful",
            "style": "chat",
            "name": request.name,
            "description": request.persona
        },
        core_values_summary=f"Agente focado em ser {request.persona}. {request.detailed_persona}"
    )

    # Formata como o AgentManager espera (Dict)
    identity_memory_dict = {
        "memory_id": SELF_MODEL_MEMORY_ID,
        "memory_type": "explicit",
        "content": {
            "text_content": f"Minha Identidade Base: {request.persona}. Detalhes: {request.detailed_persona}",
            "structured_data": initial_self_model.model_dump()
        },
        "source_type": "internal_reflection",
        "salience": "critical",
        "keywords": ["identity", "self-model", "genesis"],
        "timestamp": 0.0
    }

    # 4. Cria no Sistema de Arquivos (AgentManager) injetando a memória
    # O parametro explicit_agent_id garante que a pasta tenha o mesmo ID do banco
    agent_manager.create_agent(
        ...
        model=request.model,
        settings=request.settings,
        initial_memories=[identity_memory_dict],
        explicit_agent_id=agent_id
    )

    return {
        "agent_id": new_agent_sql.id,
        "name": new_agent_sql.name,
        "persona": new_agent_sql.persona,
        "model": new_agent_sql.model,
