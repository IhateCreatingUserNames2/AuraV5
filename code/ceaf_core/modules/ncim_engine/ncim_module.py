...
class NCIMModule:
    ...
    **INSTRUÇÕES:**
    ...
    async def get_current_self_model(self, agent_id: str) -> CeafSelfRepresentation:
        """
        Recupera o auto-modelo atual do MBS usando o ID específico do agente.
        """
        unique_id = f"{SELF_MODEL_MEMORY_ID}_{agent_id}"
        try:
            # Busca a memória de identidade usando o ID ÚNICO que a Perception criou
            mem = await self.memory.get_memory_by_id(unique_id)

            if mem and hasattr(mem, 'content') and mem.content.structured_data:
                return CeafSelfRepresentation(**mem.content.structured_data)

        except Exception as e:
            logger.error(f"NCIM: Erro ao recuperar self-model do MBS para {agent_id}: {e}")

        logger.warning(f"NCIM: Self-model [{unique_id}] não encontrado. Retornando padrão.")
            ...
            if new_self_model.version > self_model_before.version:
                try:
                    ...
                    self_model_to_save = ExplicitMemory(
                        memory_id=unique_id,  # USANDO ID COM SUFIXO
                        content=content,
                    ...
                    # Passamos o agent_id para o MBS indexar corretamente
                    await self.memory.add_specific_memory(self_model_to_save, agent_id=agent_id)
                    logger.info(f"NCIMModule: Identidade v{new_self_model.version} salva para {agent_id}.")
                except Exception as e:
                    logger.error(f"NCIMModule: Falha ao salvar identidade: {e}")
