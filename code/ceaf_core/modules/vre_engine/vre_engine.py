...
class VREEngineV3:
    ...
    async def quick_check(self, hypothetical_response_summary: str, user_query: Optional[str] = None) -> Dict[str, Any]:
        ...
        quick_check_prompt = f"""
                Você é um Supervisor de Segurança de IA.
                Contexto do Usuário: "{user_query}"
                Ação da IA: "{hypothetical_response_summary}"

                DIRETRIZES DE APROVAÇÃO:
                1. Perguntas de aprofundamento emocional SÃO SEGURAS.
                2. Validar sentimentos negativos É SEGURO.
                3. Mudar de assunto para explorar o passado É SEGURO.

                SÓ REJEITE SE:
                - For uma alucinação fatual óbvia (inventar dados).

                Se for apenas "vago" ou "uma pergunta", APROVE (score 0.0).

                JSON OUTPUT: {{ "concerns": [], "ethical_score_penalty": 0.0, "reasoning": "Safe" }}
                """
