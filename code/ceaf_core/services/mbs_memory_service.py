...
from ceaf_core.utils.geometric_laws import calculate_knowledge_density, calculate_uncertainty_pressure
...
VECTOR_SIZE = int(os.getenv("GEOMETRIC_DIMENSION"))
COLLECTION_NAME = "ceaf_memories_geometric" # Novo nome para não conflitar com legado
...
class MBSMemoryService:
    ...
    async def _ensure_collection(self):
        if self._collection_initialized: return
        ...
        try:
            ...
            if exists:
                ...
                if info.config.params.vectors.size != VECTOR_SIZE:
                    logger.critical(f"⚠️ Dimensão do modelo mudou ({info.config.params.vectors.size} -> {VECTOR_SIZE}). Recriando coleção...")
                    await self.client.delete_collection(COLLECTION_NAME)
                    exists = False # Força recriação abaixo

            if not exists:
                logger.info(f"MBS: Criando coleção '{COLLECTION_NAME}' com dimensão {VECTOR_SIZE} (Geometria do Modelo).")
            ...
            if "already exists" in str(e).lower() or "409" in str(e):
                logger.warning("Coleção já foi criada por outro processo concorrente. Prosseguindo.")
            else:
                raise
    ...
    async def search_raw_memories(
        ...
        try:
            ...
            return {"memories": [], "knowledge_density": 0.0, "uncertainty_pressure": 1.0}
        ...
        for hit in results:
            ...
            if mem_obj:
                memory_results.append((mem_obj, hit.score))
                raw_scores.append(hit.score)

        # --- APLICAÇÃO DA LEI 2: CONSCIÊNCIA DE FRONTEIRA ---
        if raw_scores:
            # Sigma=0.15 significa que a memória precisa ser bem parecida (>0.85) para contar como densidade
            rho = calculate_knowledge_density(raw_scores, sigma=0.15)
            s_max = max(raw_scores)
            psi = calculate_uncertainty_pressure(rho, s_max)
        else:
            rho = 0.0
            s_max = 0.0
            psi = 1.0  # Incerteza Total (Buraco Negro Topológico)

        # Interpretação para Logs
        boundary_status = "INTERIOR"
        if rho < 0.2:
            boundary_status = "EXTERIOR (Unknown)"
        elif rho < 0.5:
            boundary_status = "BOUNDARY (Vague)"

        return {
            "memories": memory_results,
            "knowledge_density": rho,  # ρ(q)
            "uncertainty_pressure": psi,  # Ψ(q)
            "top_similarity": s_max,
            "boundary_status": boundary_status
        }

    async def get_memory_by_id(self, memory_id: str) -> Optional[AnyMemoryType]:
        """Retrieves a single memory by ID, including its geometric vector."""
        await self._ensure_collection()
        ...
        try:
            ...
            if points:
                ...
                # Recupera o vetor que estava fora do payload e coloca de volta no objeto
                if points[0].vector:
                    payload['embedding'] = points[0].vector

                return self._reconstruct_memory_object(payload)

        except Exception as e:
            logger.error(f"MBS: Failed to retrieve memory {memory_id}: {e}")
        return None
    ...
    async def search_hybrid_memory(
        ...
        """
        Realiza uma busca vetorial e, em seguida, expande o Grafo de Conhecimento (KG)
        para trazer relações conectadas às entidades encontradas.
        """
        # 1. Busca Vetorial Padrão (Semantic Search)
        vector_results = await self.search_raw_memories(query, top_k=top_k, agent_id=agent_id)

        memories = vector_results.get("memories", [])

        # Se não achou nada, retorna logo
        if not memories:
            return vector_results

        # 2. Identificação de Entidades na Memória Recuperada
        # Vamos coletar os IDs das entidades que vieram na busca vetorial
        found_entity_ids = set()

        for mem_obj, score in memories:
            # Verifica se é um registro de entidade do KG
            if getattr(mem_obj, 'memory_type', '') == 'kg_entity_record':
                if hasattr(mem_obj, 'entity_id_str'):
                    found_entity_ids.add(mem_obj.entity_id_str)

            # Verifica também memórias explícitas que mencionam entidades (se houver metadados)
            if hasattr(mem_obj, 'metadata') and mem_obj.metadata.get('linked_entities'):
                for ent_id in mem_obj.metadata['linked_entities']:
                    found_entity_ids.add(ent_id)

        # 3. Expansão do Grafo (Graph Traversal)
        # Se achamos entidades, buscamos as relações onde elas são origem ou destino
        related_knowledge = []

        if found_entity_ids:
            logger.info(f"MBS: Expandindo grafo para entidades: {found_entity_ids}")

            # Busca relações para cada entidade encontrada
            for ent_id in list(found_entity_ids)[:5]:  # Limita a 5 entidades para não estourar contexto
                relations = await self.get_direct_relations(ent_id)
                for rel in relations:
                    # Adiciona a relação à lista de memórias se ela já não estiver lá
                    # (Aqui criamos uma tupla (memoria, score_ficticio))
                    # Damos um score alto (0.9) porque é uma conexão direta no grafo
                    related_knowledge.append((rel, 0.95))

                    # 4. Merge e Deduplicação
        # Combina os resultados vetoriais com os resultados do grafo
        all_memories = memories + related_knowledge

        # Deduplica por memory_id
        seen_ids = set()
        unique_memories = []
        ...
        vector_results["memories"] = unique_memories
        vector_results["graph_expansion_count"] = len(related_knowledge)
