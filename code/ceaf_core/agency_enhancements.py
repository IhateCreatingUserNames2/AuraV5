...
async def eval_narrative_continuity(candidate_embedding: Any, persona_embedding: Any) -> float:
    """
    Avalia a continuidade narrativa calculando a similaridade de cosseno.
    Garante conversão de list para numpy para evitar AttributeError.
    """
    if persona_embedding is None or candidate_embedding is None:
        return 0.5

    # CONVERSÃO CRÍTICA: Transforma lista em array do numpy
    cand = np.array(candidate_embedding).reshape(1, -1)
    pers = np.array(persona_embedding).reshape(1, -1)

    # Verifica se os vetores não estão vazios
    if cand.size == 0 or pers.size == 0:
        return 0.5

    # O score de similaridade
    similarity = cosine_similarity(cand, pers)

    return float((similarity[0][0] + 1) / 2)
