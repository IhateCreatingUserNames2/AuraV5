...
def extract_json_from_text(text: str) -> Optional[Union[Dict[str, Any], List[Any]]]:
    ...
    Versão OTIMIZADA para Qwen 1.7B que remove tags <think> AGRESSIVAMENTE.

    Lida com:
    - Tags de pensamento <think> gigantes (8000+ caracteres)
    - Blocos de código markdown (```json)
    - JSON puro no texto
    - Caracteres escapados e de controle
    - Strings multilinha
    ...
    # === FASE 1: LIMPEZA AGRESSIVA DE THINKING TAGS (O Pulo do Gato) ===
    # Remove TUDO entre <think> e </think> - mesmo que sejam 8000+ caracteres
    text = re.sub(r'<think>.*?</think>', '', text, flags=re.DOTALL | re.IGNORECASE)

    # Remove outras possíveis tags de reasoning (DeepSeek, Claude, etc.)
    ...
    cleaned_chars = original_len - len(text)
    if cleaned_chars > 1000:
        logger.info(f"🧹 Removed {cleaned_chars} chars from thinking tags (Qwen detected)")

    # === FASE 2: Parse direto (melhor caso) ===
    try:
        return json.loads(text)
    except json.JSONDecodeError:
        pass

    # === FASE 3: Busca por markdown blocks ===
    markdown_patterns = [
        r"```json\s*([\s\S]*?)\s*```",
        r"```\s*([\s\S]*?)\s*```",
    ]
    ...
    # === FASE 4: BUSCA AGRESSIVA - Primeiro { até último } ===
    # Esta é a estratégia mais eficiente para Qwen
    start = text.find('{')
    end = text.rfind('}')
    ...
    if start != -1 and end != -1 and end > start:
        ...
        # Tenta limpando caracteres de controle
        try:
        ...
        # Tenta corrigindo vírgulas e espaços problemáticos
        try:
    ...
    # === FASE 5: Busca por { [ em posições iniciais ===
            ...
            if not in_string:
                ...
                if balance == 0:
                    ...
                    try:
                        ...
                        logger.debug(f"✓ Extracted JSON using nested search (size: {len(potential_json_str)})")
                        return parsed_json
                    except json.JSONDecodeError:
                        try:
                            cleaned = re.sub(r'[\x00-\x1f\x7f-\x9f]', '', potential_json_str)
                            parsed_json = json.loads(cleaned)
                            logger.debug(f"✓ Extracted JSON after control char removal")
                            return parsed_json
                        except json.JSONDecodeError:
                            continue

    # === FASE 6: Fallback - último recurso ===
    logger.warning(
        f"❌ Could not extract valid JSON. "
        f"Original: {original_len} chars, After cleanup: {len(text)} chars. "
        f"Preview: {sanitize_text_for_logging(text)}"
    )
    return None
